...
- `.wpress` restores switch the backup's plugins and theme on again. All-in-One WP Migration blanks `active_plugins`, `template` and `stylesheet` in its dump and keeps them in package.json; they were left blank, so restored sites came up with the default theme and no plugins. Plugins that lock people out after a move stay off, as All-in-One WP Migration does.
- E-mail domains follow the home URL: another URL of the same host (an uploads URL moved to a CDN, say) no longer decides where addresses go.
...
- A single-site backup (`.fmw` or `.wpress`) restores as one site of a network: `wp fmw restore site.fmw --site=<name|address|id>`, or type or pick the site in the Restore dialog of Network Admin. A name or address no site has yet makes a new site with the next free ID (a folder on subdirectory networks, a subdomain or own domain on subdomain networks); an existing site's ID or full address replaces that site (a bare name that is already a site is refused), its other tables going aside with the old ones; the new ID is kept for the restore (sites made meanwhile get the next ones) and the site is listed only once its tables are live; the main site, site 1, reserved folders, backups without a database and installs with several networks are refused. Tables get the site's prefix, media moves to `uploads/sites/<id>/`, URLs and upload paths become the site's; plugins, themes and languages are restored, must-use plugins, drop-ins and other folders are left out (counted in the log). Users are merged in resumable batches: the same login or e-mail is the network's user (kept as it is, with the backup's role on the site), others are added with their own e-mail address and a free author slug, and post authors, comment users and link owners follow the new IDs (unknown users become no author). A choice that cannot work is refused in the Restore dialog before a job starts. Resumable like every restore. A pull of a single site onto a network suggests `--download-only` and then `restore --site`.
- One site of a `.fmw` network backup restores onto a single site: `wp fmw restore network.fmw --site=<id|address|url>`, or pick it in the Restore dialog. Only its tables are imported (`wp_2_*` → `wp_*`; other sites' database parts are not even copied), its media moves from `uploads/sites/<id>/` to `uploads/`, its address and media URLs become this site's while links to the other sites stay, and network-activated plugins become active plugins. Users with a role, posts, comments or links on it come along (super admins as administrators); keys shared by all sites stay; the network's e-mail domain, views and triggers stay behind; `blogs.dir` media of old networks is moved too. Without `--site`, a network backup on a single site is refused with the list of its sites.
- `.wpress` backups of whole multisite networks (All-in-One WP Migration Multisite Extension) restore onto a network of the same kind, at any address: `multisite.json` gives the sites and what each had active, the sites keep their IDs and move to the network's address like `.fmw` network backups (`--map` for subsites with their own domain), network-activated plugins come back, and media under `uploads/sites/<id>` lands in place. `wp fmw inspect` shows the network, and `wp fmw restore` previews where each site goes. Refused before anything changes: sites picked one by one, `blogs.dir` networks, a network onto a single site or the reverse.
- Pulls for multisite networks: `wp fmw pull` and the Pull screen in Network Admin copy a whole network onto a network of the same kind (subdomains or subdirectories); its sites move to this network's address as with `wp fmw restore`, and `wp fmw pull --map=old=new` gives subsites with their own domain a new one. The source reports its network (`site.network` in `GET /pull`), so a network onto a single site (or the reverse), subdomains onto subdirectories, a different main site ID or several networks are refused before a backup is made. On the source only the main site's address answers pulls; another site's address gets an error naming the network's address. The Pull screen shows the source's number of sites and kind, and pull keys can be managed in Network Admin.
...
- Addresses, paths and e-mail domains are replaced as whole matches: `example.com/shop` no longer changes `example.com/shopping`, and `example.com` no longer changes `example.com.au` or `example.community`. Find / replace pairs chosen at export (`.wpress`) still match anywhere.
